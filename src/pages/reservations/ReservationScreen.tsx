...
export function ReservationScreen({ navigation }: any) {
  ...
  // Debug: Função para testar storage
  const debugStorage = async () => {
    try {
      console.log("🔍 Debug: Verificando banco de dados...");
      const reservations = await reservationService.getUserReservations();
      console.log("📋 Debug: Reservas no banco:", reservations);
      Alert.alert("Debug", `Reservas no banco: ${reservations?.length || 0}`);
    } catch (error) {
  ...
  // Debug: Função para recarregar reservas
  const reloadReservations = async () => {
    try {
      console.log("🔄 Forçando recarregamento das reservas...");
      await loadReservations();
      Alert.alert("Recarregado", "Reservas recarregadas do banco");
    } catch (error) {
  ...
  // Debug: Função para criar reserva de teste
  const createTestReservation = async () => {
    try {
      ...
      const testReservation = {
        ...
        specialRequests: "Mesa na janela",
        status: "pending" as const,
      };

      console.log("📋 Reserva de teste:", testReservation);

      // Criar usando o serviço
      const newReservation = await reservationService.createReservation(
        testReservation
      );
      console.log("✅ Reserva de teste criada:", newReservation);

      Alert.alert("Teste", `Reserva de teste criada! ID: ${newReservation.id}`);
      ...
      Alert.alert(
        "Erro",
        `Falha ao criar reserva de teste: ${
          error instanceof Error ? error.message : "Erro desconhecido"
        }`
      );
    }
  ...
  // Debug: Função para limpar todas as reservas
  const clearAllReservations = async () => {
    try {
      ...
      Alert.alert(
        "Limpo",
        `Todas as ${reservations.length} reservas foram removidas!`
      );
    } catch (error) {
      console.error("❌ Erro ao limpar reservas:", error);
      Alert.alert(
        "Erro",
        `Falha ao limpar reservas: ${
          error instanceof Error ? error.message : "Erro desconhecido"
        }`
      );
    }
      ...
      >
        <View style={localStyles.header}>
          ...
          <View style={localStyles.headerButtons}>
            ...
            >
              <Text style={styles.buttonTextSecondary}>Debug</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={reloadReservations}
              style={[
                styles.buttonSecondary,
                { marginRight: 8, paddingHorizontal: 8 },
              ]}
            >
              <Text style={styles.buttonTextSecondary}>Recarregar</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={createTestReservation}
              style={[
                styles.buttonSecondary,
                { marginRight: 8, paddingHorizontal: 8 },
              ]}
            >
              <Text style={styles.buttonTextSecondary}>Teste</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={clearAllReservations}
              style={[
                styles.buttonSecondary,
                { marginRight: 8, paddingHorizontal: 8 },
              ]}
